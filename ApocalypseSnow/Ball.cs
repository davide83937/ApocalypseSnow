...
public class Ball:DrawableGameComponent
{
    ...
    private static extern Vector parabolic_motion(float startPositionX, float startPositionY, float startVelocityX,
        float startVelocityY, float gameTime, float deltaTime);
    ...
    {
        spriteBatch.Draw(_texture, _start_position, Color.White);
    }

    public override void Update(GameTime gameTime)
    {
        float deltaTime = (float)gameTime.ElapsedGameTime.TotalSeconds;
        var v = parabolic_motion(_start_position.X, _start_position.Y, _start_speed.X, _start_speed.Y, _ball_time, deltaTime);
        _start_position.X = v._x;
        _start_position.Y = v._y;
